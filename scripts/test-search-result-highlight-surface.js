...
function getCssRuleBlock(source, selector) {
  // Match rule selectors whitespace-insensitively so formatting line breaks
  // in the stylesheet do not break the lookup.
  const strippedSelector = selector.replace(/\s+/g, "");
  let strippedSource = "";
  const indexMap = [];
  for (let i = 0; i < source.length; i += 1) {
    if (!/\s/.test(source[i])) {
      strippedSource += source[i];
      indexMap.push(i);
    }
  }
  const start = strippedSource.indexOf(`${strippedSelector}{`);
  ...
  // Normalize whitespace inside the block so downstream assertions match
  // regardless of formatter line breaks.
  return source
    .slice(indexMap[start], indexMap[end] + 1)
    .replace(/\s+/g, " ")
    .replace(/\(\s+/g, "(")
    .replace(/\s+\)/g, ")");
}
...
assert.match(
  getCssRuleBlock(overlayCss, overlayDarkActiveFaviconSelector),
...
assert.match(
  getCssRuleBlock(newtabHtml, newtabDarkActiveFaviconSelector),
  /background-color:\s*rgba\(255,\s*255,\s*255,\s*0\.1\);/,
