...
function assertDirectNavigationDelegatesToShared(relativePath) {
  ...
  assert.ok(
    /function getDirectNavigationUrl\(input\)\s*\{[\s\S]*?typeof (?:SEARCH_UTILS|searchUtils)\.getDirectNavigationUrl === 'function'[\s\S]*?(?:SEARCH_UTILS|searchUtils)\.getDirectNavigationUrl\(input\)/.test(
      source,
    ),
...
function assertKeywordOnlySuggestionsKeepSearchActionFirst(relativePath) {
  ...
  assert.match(
    source,
    /if \(preferAutocompleteFirst &&[\s\S]*?SEARCH_UTILS\.composeNavigationFirstSlate\(allSuggestions/,
    `${relativePath} should compose navigation results before the search action in autocomplete-first mode`,
  );
}
...
assert.strictEqual(
  search.matchesSearchQueryText(
    { title: "X Home", url: "https://example.com/" },
    shortAsciiContext,
  ),
  true,
...
assert.strictEqual(
  releaseCoverage.allMatched,
  true,
  "title + path tokens should cover multi-term queries",
);

const rootScore = score(
  { title: "Lumno", url: "https://lumno.kubai.design/" },
  "lumno release",
  "topSite",
);
const releaseScore = score(
  { title: "Release | Lumno", url: "https://lumno.kubai.design/release/" },
  "lumno release",
  "history",
);
assert.ok(
  releaseScore > rootScore,
  "specific release page should outrank root for path-intent queries",
);
...
assert.strictEqual(
  search.getSearchSuggestionClusterInfo("https://mall.xiaohongshu.com/finance/cashier/web")
    .category,
  "utility",
...
const xiaohongshuDiverse = search.applySearchSuggestionHostDiversity(
  [xiaohongshuDirect, ...xiaohongshuCandidates.slice(0, 3), xiaohongshuOpenTab].filter(Boolean),
  { context: xiaohongshuContext },
);
assert.ok(
  xiaohongshuDiverse.length <= 3 &&
    xiaohongshuDiverse.every(
      (item) => search.getSearchSuggestionFamilyKey(item.url) === "xiaohongshu.com",
    ),
...
const defaultSearchFirstSlate = search
  .composeSearchFirstSuggestionSlate(searchFirstQuotaInput, { limit: 10 })
  .slice(0, 10);
assert.deepStrictEqual(
...
assert.deepStrictEqual(
  search
    .composeSearchFirstSuggestionSlate(searchFirstQuotaInput, { limit: 5 })
    .slice(0, 5)
    .map((item) => item.type),
...
for (let i = 0; i < 12; i += 1) {
  selectionStats = search.recordSearchSelectionInStats(
    selectionStats,
    {
      query: "ProjectX",
...
const selectedScore =
  search.calculateSearchRelevanceScore(
    selectedCandidate,
    "history",
    selectionContext,
    selectionOptions,
...
const topSiteScore = search.calculateSearchRelevanceScore(
  topSiteCandidate,
  "topSite",
  selectionContext,
  selectionOptions,
...
brandDeepSelectionStats = search.recordSearchSelectionInStats(
...
const learnedNavigationMatch = search.promoteStrongNavigationMatch(
  learnedNavigationList,
  "ProjectX",
);
assert.strictEqual(
...
assert.deepStrictEqual(
  search
    .limitSearchSuggestionsForDisplay(displaySuggestionItems, { limit: 5 })
    .map((item) => item.title),
...
assert.deepStrictEqual(
  search.normalizeSearchEngineSuggestions(
    ["foo", "foo bar", "Foo Bar", "foo baz", "foo qux"],
    "foo",
    { limit: 2 },
  ),
  ["foo bar", "foo baz"],
...
assert.strictEqual(
  search.getSearchEngineSuggestionPolicy(search.buildSearchQueryContext("什么东西"), [], {
    maxEngineSuggestions: 5,
  }).limit,
  5,
...
assert.strictEqual(
  promoted.title,
  "Example Home",
  "strong navigation promotion should choose representative pages",
);
assert.strictEqual(
  navList[0].title,
  "Example Home",
  "strong navigation promotion should mutate the list consistently",
);
...
const complexPromoted = search.promoteStrongNavigationMatch(
  complexNavigationList,
  complexUrlInput,
  {
    getDirectNavigationUrl: testDirectNavigationUrl,
...
assert.deepStrictEqual(
  search.findSearchOpenTabMatchIndex([directComplexUrlSuggestion, unrelatedOpenTabSuggestion], {
...
assert.deepStrictEqual(
  search.findSearchOpenTabMatchIndex(
    [{ ...directComplexUrlSuggestion, _xMatchedTabId: 99 }, unrelatedOpenTabSuggestion],
    {
      rawQuery: complexUrlInput,
      primaryHighlightIndex: 0,
      openTabQuickSwitchEnabled: true,
...
assert.strictEqual(
  xRootSuggestion.title,
  "X",
  "configured site roots should use stable direct titles",
);
...
assert.strictEqual(
  xProfileSuggestion.title,
  "(1) نوف | Nouf (@Nouf0633) / X",
  "configured titles should not replace profile paths",
);
...
assert.strictEqual(
  xBookmarkSuggestion.title,
  "My X bookmark",
  "bookmark titles should remain user controlled",
);

const dedupUrl = search.buildSearchDedupUrlKey(
  "https://www.example.com/docs/?utm_source=x&ref=abc&keep=1#section",
);
assert.strictEqual(
  dedupUrl,
  "https://example.com/docs?keep=1",
  "dedupe URL should drop tracking params and hashes",
);
...
assert.ok(
  search.isInteractiveSiteSearchProvider(geminiBase),
  "Gemini provider should be interactive",
);

const defaultSearchEngines = search
...
assert.strictEqual(
  customizedGemini.action,
  "openAndSubmit",
  "customized provider should inherit action",
);
assert.strictEqual(
  customizedGemini.submitStrategy,
  "geminiPrompt",
  "customized provider should inherit submit strategy",
);
...
assert.strictEqual(
  merged[0]._xIsCustom,
  true,
  "merged custom providers should retain their custom identity",
);
...
assert.strictEqual(
  search.findSiteSearchProviderKeyConflict("db", [{ key: "db", name: "Douban" }], "db"),
  null,
  "editing a provider should allow it to retain its current trigger",
);

const wechatProvider = search
  .getDefaultSiteSearchProviders()
  .find((provider) => provider.key === "wx");
assert.strictEqual(
...
assert.strictEqual(
  search.getSiteSearchTriggerCandidate("macked", [titledProvider], {
    type: "topSite",
    title: "MacKed - Mac Apps Library",
...
assert.strictEqual(
  search.getSiteSearchTriggerCandidate("macked", [titledProvider], {
    type: "topSite",
    title: "MacKed - Search Results",
    url: "https://downloads.example.com/search?q=macked",
  }),
  null,
...
assert.strictEqual(
  replaceParse.query,
  "lum",
  "text after the leading token run should be the query",
);
...
assert.strictEqual(
  pendingParse.pending.text,
  "@h",
  "pending fragment should capture the typed text",
);
...
assert.strictEqual(
  ...
  "history",
  "canonicalization should report the winning scope",
);
